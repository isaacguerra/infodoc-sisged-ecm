...
{
    public $template_info, $items_list, $templateProcessor, $column_total;
    ...
    {     
        ...
        if(!is_file(DIR_WS_TEMPLATES . $this->template_info['filename'])){
            die(TEXT_ERROR . ' ' . TEXT_EXT_EXPORT_TEMPLATES . ' => ' . DIR_WS_TEMPLATES .  $this->template_info['filename']  . '. <font color="red">'. TEXT_FILE_NOT_FOUND . '</font>');
        }
        
        //temp file
        $temp_filename = time() . '-' . $items_id . '-' . $app_user['id'] . '-' . $entities_id   . '-' . $this->template_info['id'] . '.docx';
    ...
    function prepare_template_blocks()
    {
        ...
        {  
            ...
            //mysql query
            if($blocks['extra_type']=='table')
            {
                $this->prepare_template_table_mysql_query($blocks, $item);
                
                continue;
            }
            
            //php
            if($blocks['extra_type']=='php')
            {
                $this->prepare_template_php_value($blocks, $item);
            ...
            switch($blocks['field_type'])
            {
                ...
                case 'fieldtype_textarea':
                    $fontStyle = [
                        'name' => (strlen($block_settings->get('font_name')) ? $block_settings->get('font_name') : 'Times New Roman') . $this->add_text_break(1),
                    ...
                    while ($this->templateProcessor->getVariableCount()[$blocks['id']]??false) 
                    {                    
                ...
                case 'fieldtype_php_code': 
                    $this->prepare_fieldtype_php_code_blocks($blocks['id'],$field,$output_value_html,$block_settings);
                    break;
                default:                                                                                                                       
                    $this->templateProcessor->setValue($blocks['id'],$this->prepareValue($output_value));
    ...
    //Added text-break-[d] as font name to fixt text-break in html export
    function add_text_break($value)
    {
        global $app_module_action;
        
        if(strlen($value) and in_array($app_module_action,['export_pdf','print']))
        {
            return ' text-break-' . $value;
        }        
    }
    
    function prepare_template_php_value($blocks, $item)
    {
        global $app_entities_cache, $app_fields_cache, $app_user, $app_users_cache, $app_choices_cache, $app_global_choices_cache, $app_access_groups_cache;
        ...
        $fontStyle = [
                'name' => (strlen($settings->get('font_name')) ? $settings->get('font_name') : 'Times New Roman'),
                'size' => (strlen($settings->get('font_size')) ? $settings->get('font_size') : '12'), 
                'color' => $settings->get('font_color')            
            ];
        
        //print_rr($fontStyle);
        //exit();
                
        $output = preg_split('/<br>|<br \/>|<\/p>|<\/div>/',$output_value);
        
        $output = array_map(function($v){ return strip_tags(trim($v)); },$output);
        $output = array_filter($output, function($v){ return strlen($v) ? true : false; });
        //print_rr($output);
        //exit();
        ...
        {
            $text->addText(strip_tags(trim($value)), $fontStyle); 
            if($k<(count($output)-1))
            {
                $text->addTextBreak();                                   
            }
    ...
    function prepare_comments_list()
    {
        ...
        {          
            $name = ($app_users_cache[$comments['created_by']]['name']??'') . ' - ' . format_date_time($comments['date_added']);
            ...
            $comments_fields_query = db_query("select f.*,ch.fields_value from app_comments_history ch, app_fields f where comments_id='" . db_input($comments['id']) . "' and f.id=ch.fields_id order by ch.id");
    ...
    function prepare_img_from_html($block_id,$html, $field)
    {
        ...
        if(strlen($html))
        {
            ...
            elseif($field['type']=='fieldtype_qrcode')
            {
                $img = getimagesize($tmp_filepath);
                              
                $options = [
                    'path'=>$tmp_filepath,
                    'ratio'=>false,
                    'width' => floor($img[0]/2) ,
                    'height' => floor($img[1]/2),
                    ];           
    ...
    function prepare_template_table_mysql_query($blocks, $item)
    {
       $block_id = $blocks['id'];
       $block_settings = new settings($blocks['settings']); 
       
       if(!strlen($block_settings->get('mysql_query')))
       {
           return false;
       }
       
       $this->prepare_template_table_mysql_query_blocks($block_id, $block_settings);
    }
    
    function prepare_template_table_mysql_query_blocks($parent_block_id,$parent_block_settings)
    {
        global $app_fields_cache, $app_num2str, $app_user;
        ...
        $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where  block_type='body_cell' and b.templates_id = " . $this->template_info['id'] . " and b.parent_id = " . $parent_block_id . "  order by b.sort_order, b.id",false);
        ...
        while($blocks = db_fetch_array($blocks_query))
        {
            ...
            $cell->addText($this->prepareValue($cell_name),$this->prepare_font_style($settings->get('heading_font_style'),$parent_block_settings,$settings->get('heading_font_size')));
        }
        ...
        if($parent_block_settings->get('column_numbering')==1)
        {
            ...
            if($parent_block_settings->get('line_numbering')==1)
            {
                ...
                $cell = $cell->addTextRun($this->prepare_alignment('center'));
                $cell->addText($this->prepareValue($count),$this->prepare_font_style([],$parent_block_settings));
                $count++;
            }
        ...
        $current_item = $this->items_list[$this->template_info['entities_id']];
        
        $mysql_query = str_replace(['[current_user_id]','[current_item_id]'],[$app_user['id'],$current_item['id']],$parent_block_settings->get('mysql_query')); 
        
        foreach($current_item as $k=>$v)
        {
            if(strstr($k,'field_'))
            {
                $field_id = str_replace('field_','',$k);
                $mysql_query = str_replace("[{$field_id}]",$v??'',$mysql_query);
            }
        }
        
        
        //print_rr($mysql_query);
        //exit();
        
        $item_query = db_query($mysql_query);
        $number_of_rows = db_num_rows($item_query);
        ...
        while($item = db_fetch_array($item_query))
        {
            ...
            {                
                $cell = $wordTable->addCell(null,$this->prepare_cell_settings('',$table_bg_color));
                $cell = $cell->addTextRun($this->prepare_alignment('center'));
                $cell->addText($this->prepareValue($item_count),$this->prepare_font_style([],$parent_block_settings));
            }
            ...
            while($blocks = db_fetch_array($blocks_query))
            {
                ...
                $value  = (strlen($column) and isset($item[$column])) ? $item[$column] : '';
                ...
                $cell_settings = $this->prepare_cell_settings('',$table_bg_color);
                ...
                $cell->addText($this->prepareValue($value),$this->prepare_font_style($settings->get('content_font_style'),$parent_block_settings,$settings->get('content_font_size')));                                                                                                                                  
            }
        ...
        //hide block if emtpy
        if($number_of_rows==0 and $parent_block_settings->get('hide_if_empty')==1)
        {
            $this->templateProcessor->setValue($parent_block_id,''); 
        }
        else
    ...
    static function render_total_helper($block_id)
    {
        ...
        $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b  where  block_type='body_cell'  and b.parent_id = " . $block_id . " order by b.sort_order, b.id",false);
        while($blocks = db_fetch_array($blocks_query))
        {       
            $settings = new \settings($blocks['settings']);
    ...
    //prepare field value for mysql query
    ...
    function prepare_template_output_items($item_query,$block_id,$entity_id,$block_settings)
    {
        ...
        switch($block_settings->get('display_us'))
        {
            ...
            case 'list':
                $fontStyle = [
                'name' => (strlen($block_settings->get('font_name')) ? $block_settings->get('font_name') : 'Times New Roman') . $this->add_text_break($block_settings->get('empty_row')),
                'size' => (strlen($block_settings->get('font_size')) ? $block_settings->get('font_size') : '12'),                
                ];
    ...
    {
        $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select($entity_id, '') . " from app_entity_" . $entity_id . " e where parent_id={$item_id} order by e.sort_order, e.id"); 
        while($item = db_fetch_array($item_query))
        {
        ...
        {                                              
            ...
            if(db_num_rows($blocks_query))
            {
                ...
                while($blocks = db_fetch_array($blocks_query))
                {
                    ...
                    {                        
                        case 'php_code':                            
                            $cell_value = $this->render_body_cell_php_value($this->items_list[$this->template_info['entities_id']], $blocks, $this->column_total);
                            //apply number format
                    ...
                    $cell = $cell->addTextRun($this->prepare_alignment($settings->get('heading_alignment')));
                    $cell->addText($this->prepareValue($cell_value),$this->prepare_font_style($settings->get('heading_font_style'),$parent_block_settings,$settings->get('heading_font_size')));                
                }
            }
    ...
    function prepare_template_table_blocks($parent_block_id,$entity_id, $output,$parent_block_settings)
    {
        ...
        foreach($output as $item_count=>$item)
        {
            ...
            {                
                $cell = $wordTable->addCell(null,$this->prepare_cell_settings('',$table_bg_color));
                $cell = $cell->addTextRun($this->prepare_alignment('center'));
                $cell->addText($this->prepareValue($item_count+1),$this->prepare_font_style([],$parent_block_settings));
            }
            ...
            while($blocks = db_fetch_array($blocks_query))
            {
                ...
                switch($blocks['field_type'])
                {
                    ...
                    case 'fieldtype_items_by_query':     
                        ...
                        $font = $this->prepare_font_style([],$parent_block_settings);
                        
                        $style = 'style="line-height: 1; font-family: \"' . $font['name'] . '\"; font-size: ' . $font['size']. 'pt; color: ' . $font['color'] . '"';
                        $style = 'style="font-family: \'' . $font['name'] . '\'; font-size: ' . ($font['size']>0 ? $font['size'] : 12) . 'pt; line-height: normal; margin:0; padding:0"';
                                                
                        $output_value = str_replace(['&','&amp;',"\n\r","\r","\n"],' ',trim(strip_tags($output_value_html,'<li>')));
                        $output_value = str_replace(['<li>','</li>'],['<p ' . $style . '>','</p>'],$output_value);
                        $output_value = preg_replace('/<\/p>(\s+)/','</p>',$output_value);
                ...
                if($blocks['field_type']=='fieldtype_items_by_query')
                {
                    continue;
                }
                ...
                $cell = $wordTable->addCell($cell_width,$cell_settings);
                $cell = $cell->addTextRun($this->prepare_alignment($settings->get('content_alignment')));
                $cell->addText($this->prepareValue($output_value),$this->prepare_font_style($settings->get('content_font_style'),$parent_block_settings,$settings->get('content_font_size')));
            ...
            {                               
                ...
                if(is_array($settings->get('fields')))
                {
                    ...
                    foreach($settings->get('fields') as $field_id)
                    {
                        ...
                        {
                            case 'fieldtype_items_by_query':
                                $output_value = str_replace(['&','&amp;'],' ',trim(strip_tags($output_value,'<li>')));
                                
                                $output_value_array = explode('<li>',$output_value);
                                $output_value = '';
                                $count =1;
                                foreach($output_value_array as $v)
                                {
                                    if($count==count($output_value_array))
                                    {
                                        $output_value .= trim(str_replace('</li>','',$v));
                                    }
                                    else
                                    {
                                        $output_value .= trim(str_replace('</li>','<br />',$v));
                                    }
                                ...
                                break;
                            default:
                                $output_value = str_replace(['&','&amp;'],' ',trim(strip_tags($output_value)));
                                break;
                        }                                                
                       ...
                       //exit();
                        ...
                        if($settings->get('display_field_names')==1)
                        {
                            ...
                            $html .= '<td ' . $css . ($settings->get('field_name_column_width')>0 ? ' width="' . $settings->get('field_name_column_width') . '"':'') . '><b>' . trim(strip_tags(fields_types::get_option($field['type'], 'name',$field['name']))) . '</b></td>';
                        }
                        
                        //$css1 = str_replace('style="','style="width: 50%;',$css);                                                
                        
                        $html .= '<td ' . $css . ($settings->get('value_column_width')>0 ? ' width="' . $settings->get('value_column_width') . '"':'') .  ' >' . $output_value . '</td>';
    ...
    function prepare_font_style_css($settings,$table_settings)
    {
        ...
        if(strlen($settings->get('content_alignment')) and $settings->get('content_alignment')!='left') $font_style['text-align'] = $settings->get('content_alignment');
    ...
    {        
        ...
        if($block_settings->get('display_us')=='list')
        {
            $fontStyle = [
                'name' => (strlen($block_settings->get('font_name')) ? $block_settings->get('font_name') : 'Times New Roman') . $this->add_text_break($block_settings->get('empty_row')),
                'size' => (strlen($block_settings->get('font_size')) ? $block_settings->get('font_size') : '12'),
                'bold' =>$this->isBold($block_settings),
                'italic' =>$this->isItalic($block_settings),
                'underline' =>$this->isUnderline($block_settings),
            ];
    ...
    function isBold($settings)
    {
        return is_array($settings->get('font_style')) ? in_array('bold',$settings->get('font_style')) : false;
    }
    
    function isItalic($settings)
    {        
        return is_array($settings->get('font_style')) ? in_array('italic',$settings->get('font_style')) : false;
    }
    
    function isUnderline($settings)
    {        
        return is_array($settings->get('font_style')) ? in_array('underline',$settings->get('font_style')) : false;        
    }
        
    function prepare_fieldtype_php_code_blocks($parent_block_id,$entity_id, $field_value,$block_settings)
    {
        $output = [];
                
        $output = preg_split('/<br>|<br \/>|<\/p>|<\/div>/',$field_value);
        ...
        if(count($output)>1 and $block_settings->get('display_us')=='list')
        {   
            $fontStyle = [
                'name' => (strlen($block_settings->get('font_name')) ? $block_settings->get('font_name') : 'Times New Roman') . $this->add_text_break($block_settings->get('empty_row')),
                'size' => (strlen($block_settings->get('font_size')) ? $block_settings->get('font_size') : '12'),                
    ...
    function prepare_template_entity_list_blocks($parent_block_id,$entity_id, $items_id_list,$block_settings)
    {
        ...
        if(strlen($items_id_list))
        {
            ...
            while($item = db_fetch_array($item_query))
            {
                ...
                {
                    $text_pattern = new fieldtype_text_pattern();
                    $output[] = $this->prepareValue(strip_tags($text_pattern->output_singe_text($block_settings->get('pattern'), $entity_id, $item)));
                }
                else
                {
                    $output[] = $this->prepareValue(strip_tags(items::get_heading_field($entity_id, $item['id'],$item)));
                }
            }
        }
        
        if($output_count = count($output))
        {   
            $fontStyle = [
                'name' => (strlen($block_settings->get('font_name')) ? $block_settings->get('font_name') : 'Times New Roman') . $this->add_text_break($block_settings->get('empty_row')),
                'size' => (strlen($block_settings->get('font_size')) ? $block_settings->get('font_size') : '12'),                
    ...
    {     
        ...
        {            
            //and f.id not in (select fields_id from app_ext_items_export_templates_blocks where block_type = 'parent' and templates_id=" . $template_id . " " . ($fields_id>0 ? " and fields_id!=" . $fields_id :"") . ")
            $fields_query = fields::get_query($entity_id," and f.type not in ('fieldtype_action','fieldtype_parent_item_id') ");
    ...
    {
        $block_query = db_query("select * from app_ext_items_export_templates_blocks where templates_id='" . $templates_id . "' and fields_id=0");
    ...
    {
        \PhpOffice\PhpWord\Settings::setTempDir(DIR_FS_TMP);
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor(DIR_WS_TEMPLATES . $filename);
        foreach($id_to_replace as $block_id=>$new_block_id)
        {
            $templateProcessor->setValue($block_id,'${' . $new_block_id . '}');
            $templateProcessor->setValue($block_id .':count' ,'${' . $new_block_id . ':count}');
            $templateProcessor->setValue($block_id .':count_text' ,'${' . $new_block_id . ':count_text}');
        }
        
        $templateProcessor->saveAs(DIR_WS_TEMPLATES . $filename);
    }
}